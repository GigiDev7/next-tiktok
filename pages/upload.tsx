...
const Upload = () => {
  ...
  >();
  const [wrongFilType, setWrongFileType] = useState(false);
          ...
          >
            {topics.map((item) => {
              return (
                <option
                  value={item.name}
                  className="cursor-pointer outline-none capitalize bg-white text-gray-700 text-md p-2 hover:bg-slate-300"
                  key={item.name}
                >
                  {item.name}
                </option>
